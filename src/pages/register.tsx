      ...
      <div style={{ flex: '1 1 380px', width: '100%', maxWidth: step === 3 ? 720 : 440, background: '#fff', borderRadius: 16, padding: 32, boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        {step === 3 && (
          <div style={{ marginTop: 24, textAlign: 'center' }}>
            ...
            <div style={{ marginTop: 16 }}>
              <Link to="/login" style={{ display: 'inline-block', padding: '12px 32px', background: '#4285f4', color: '#fff', borderRadius: 8, textDecoration: 'none' }}>立即登录</Link>
            </div>
          </div>
        )}
        ...
        {step < 3 && (
          <div style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: '#666' }}>
            已有账号？<a href="/login" style={{ color: '#4285f4', textDecoration: 'none', marginLeft: 4 }}>立即登录</a>
          </div>
        )}
      </div>
      ...
      {step < 3 && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { icon: '💬', title: '论坛热帖', desc: '和大家一起聊技术、日常与脑洞', href: '/forum' },
              { icon: '📝', title: '投稿广场', desc: '用 Markdown 写下你的教程与故事', href: '/submissions' },
              { icon: '🌙', title: '朋友圈动态', desc: '记录此刻，看看别人的碎碎念', href: '/moments' },
            ].map((it) => (
              <a
                key={it.href}
                href={it.href}
